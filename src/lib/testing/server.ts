...
import type { StripePaymentIntent } from "../payment/stripe";
...
const mockVNPayResponse: VNPayResponse = {
  paymentUrl:
    "https://sandbox.vnpayment.vn/paymentv2/vpcpay.html?vnp_Amount=55000000&vnp_Command=pay",
...
export const handlers = [
  ...
  http.post("/api/payment/create", async ({ request }) => {
    ...
    if (paymentMethod === "vnpay") {
      ...
      return HttpResponse.json(mockStripeResponse);
    } else {
      return HttpResponse.json(
  ...
  http.post(
    "https://byidgbgvnrfhujprzzge.supabase.co/rest/v1/*",
    async ({ request }) => {
