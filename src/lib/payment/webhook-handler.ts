...
class WebhookHandler {
  ...
  async handleMoMoWebhook(
    ...
    try {
      ...
      const updatedOrder = await orderManager.confirmPayment(
        verification.orderId,
        verification.transactionId?.toString() ||
          webhookData.transId.toString(),
        "momo"
      );
  ...
  private async generateInvoiceForOrder(orderId: string): Promise<void> {
    try {
      ...
      if (customerInfo) {
        const invoice = invoiceGenerator.createInvoiceFromOrder(
          order,
          customerInfo
        );
  ...
  private async sendPaymentConfirmation(
    ...
    try {
      ...
      console.log(
        `Payment confirmation sent for order ${orderId} via ${gateway}`
      );
